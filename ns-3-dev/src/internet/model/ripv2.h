...
/**
 * \defgroup Ripv2 Ripv2
 *
 * The Ripv2 protocol (\rfc{2080}) is a unicast-only Ipv4 IGP (Interior Gateway Protocol).
 ...
 * data flows.
 *
 * Ripv2 implements the Bellman-Ford algorithm (although the RFC does not state it).
 ...
 *
 * For the Ripv2 protocol, the exact convergence time is shorter, thanks to the
...
/**
 * \ingroup Ripv2
 * \brief Ripv2 Routing Table Entry
 */
class Ripv2RoutingTableEntry : public Ipv4RoutingTableEntry
{
public:
  ...
  Ripv2RoutingTableEntry (void);
  /**
   * \brief Constructor
   * \param network network address
   * \param subnetMask network subnetmask
   * \param nextHop next hop address
   * \param interface interface index
   */
  Ripv2RoutingTableEntry (Ipv4Address network, Ipv4Mask subnetmask, Ipv4Address nextHop, uint32_t interface);
  /**
   * \brief Constructor
   * \param network network address
   * \param subnetMask network subnetmask
   * \param interface interface index
   */
  Ripv2RoutingTableEntry (Ipv4Address network, Ipv4Mask subnetmask, uint32_t interface);

  virtual ~Ripv2RoutingTableEntry ();
 ...
 */
std::ostream& operator<< (std::ostream& os, Ripv2RoutingTableEntry const& route);

/**
 * \ingroup Ripv2
 *
 * \brief Ripv2 Routing Protocol, defined in \rfc{2080}.
 */
class Ripv2 : public Ipv4RoutingProtocol
{
public:
  // /< C-tor
  Ripv2 ();
  ...
  virtual void NotifyAddRoute (Ipv4Address dst, Ipv4Mask mask, Ipv4Address nextHop,
                               uint32_t interface);
  virtual void NotifyRemoveRoute (Ipv4Address dst, Ipv4Mask mask, Ipv4Address nextHop,
                                  uint32_t interface);
   ...
   */
  uint32_t GetInterfaceMetric (uint32_t interface) const;
   ...
   */
  void SetInterfaceMetric (uint32_t interface, uint32_t metric);
...
private:
  /// Container for the network routes - pair Ripv2RoutingTableEntry *, EventId (update event)
  typedef std::list<std::pair <Ripv2RoutingTableEntry *, EventId> > Routes;
  /// Iterator for container for the network routes
  typedef std::list<std::pair <Ripv2RoutingTableEntry *, EventId> >::const_iterator RoutesCI;
  typedef std::list<std::pair <Ripv2RoutingTableEntry *, EventId> >::iterator RoutesI;
  /**
   * \brief Receive Ripv2 packets.
   *
   ...
   */
  void HandleRequests (Ripv2Header hdr, Ipv4Address senderAddress, uint16_t senderPort, uint32_t incomingInterface, uint8_t hopLimit);

  /**
   * \brief Handle Ripv2 responses.
   *
   * \param hdr message header (including RTEs)
   * \param senderAddress sender address
   * \param incomingInterface incoming interface
   * \param hopLimit packet's hop limit
   */
  void HandleResponses (Ripv2Header hdr, Ipv4Address senderAddress, uint32_t incomingInterface, uint8_t hopLimit);
   ...
   */
  void InvalidateRoute (Ripv2RoutingTableEntry *route);

  /**
   * \brief Delete a route.
   * \param route the route to be removed
   */
  void DeleteRoute (Ripv2RoutingTableEntry *route);
  ...
  std::map<uint32_t, uint32_t> m_interfaceMetrics; // Map of interface metrics
